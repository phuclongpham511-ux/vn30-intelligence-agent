...
test("raw provider names never become English display names", () => {
  for (const company_name of ["CTCP Sữa Việt Nam", "CTCP Cao su Viet Nam", "Company raw name", "FPT Corporation"]) {
...
test("verified English metadata works for any ticker without a fixed universe", () => {
  assert.equal(englishCompanyName({ company_name: "Tên thô", display_name_en: "Example International Corporation" }), "Example International Corporation");
  assert.equal(englishCompanyName({ display_name_en: "  " }), null);
});
test("missing metrics are not shown as zero", () => {
  for (const value of [null, undefined, NaN, Infinity]) assert.equal(formatNumber(value), "—");
