...
export default function TechnicalMetrics({ market }: { market: Market }) {
  return <section><div className="mb-4 flex items-center justify-between"><h2>Technical overview</h2><span className="text-[10px] text-muted-foreground">Calculated from daily OHLCV</span></div>
    <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
      ...
      <div className="panel p-4"><h3 className="mb-1 flex items-center gap-2 text-xs"><Gauge size={14} className="text-muted-foreground"/>Momentum</h3><dl><MetricRow label="RSI 14" value={market.rsi14}/></dl><p className="mt-3 text-[10px] text-muted-foreground">Wilder smoothing · 14 sessions</p></div>
      <div className="panel p-4"><h3 className="mb-1 flex items-center gap-2 text-xs"><Activity size={14} className="text-muted-foreground"/>Activity</h3><dl><MetricRow label="Relative volume" value={market.relative_volume_20d} suffix="×"/><MetricRow label="20D avg. volume" value={market.avg_volume_20d}/></dl></div>
