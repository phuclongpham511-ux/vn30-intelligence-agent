...
export default function StockHeader({ stock, market }: { stock: Stock; market: Market }) {
  ...
  return <div>
    ...
    <div className="mt-5 flex flex-wrap items-center gap-x-4 gap-y-2 text-[11px] text-muted-foreground"><span className="flex items-center gap-1.5"><Clock3 size={12}/>As of {formatDate(market.as_of)}</span><span>Source: {market.source}</span><span>Daily data Â· Not realtime</span></div>
  </div>;
}
